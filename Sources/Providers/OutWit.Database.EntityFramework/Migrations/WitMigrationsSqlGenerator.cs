...
public sealed class WitMigrationsSqlGenerator : MigrationsSqlGenerator
{
    ...
    /// <summary>
    /// The store type for a column operation, including the size the model declares.
    /// </summary>
    /// <param name="operation">The column being added.</param>
    /// <returns>The store type.</returns>
    /// <remarks>
    /// Only the CLR type used to be consulted, so MaxLength = 16 became TEXT and
    /// Precision/Scale = 18,4 became DECIMAL - the model's own constraints dropped on the way to
    /// the DDL, with nothing said. Note this makes the schema state the size; enforcing it is a
    /// separate matter, recorded against the engine.
    /// </remarks>
    private string GetColumnType(ColumnOperation operation)
    {
        var baseType = GetColumnType(operation.ClrType);

        if (operation.MaxLength is > 0 && baseType is "TEXT")
        {
            return $"VARCHAR({operation.MaxLength.Value.ToString(CultureInfo.InvariantCulture)})";
        }

        if (operation.MaxLength is > 0 && baseType is "BLOB")
        {
            return $"VARBINARY({operation.MaxLength.Value.ToString(CultureInfo.InvariantCulture)})";
        }

        if (operation.Precision is > 0 && baseType is "DECIMAL")
        {
            return operation.Scale is > 0
                ? $"DECIMAL({operation.Precision.Value},{operation.Scale.Value})"
                : $"DECIMAL({operation.Precision.Value})";
        }
    ...
    /// <remarks>
    /// Every conversion here is pinned to the invariant culture, and both halves of that matter.
    /// The fallback used to be a plain ToString(), so a decimal default of 1.5 was written as
    /// <c>DEFAULT 1,5</c> on a machine with a comma decimal separator - a migration generated by one
    /// developer that is corrupt SQL for everyone. Less obvious: ':' and '/' inside a custom
    /// date/time format are the *culture's* time and date separators, not literals, so even the
    /// explicit "yyyy-MM-dd HH:mm:ss" shifted with the culture until it was given
    ...
    private string GenerateSqlLiteral(object value)
    {
        ...
        return value switch
        {
            ...
            DateTimeOffset dto => $"'{dto.ToString("yyyy-MM-dd HH:mm:sszzz", culture)}'",
            DateOnly d => $"'{d.ToString("yyyy-MM-dd", culture)}'",
            TimeOnly t => $"'{t.ToString("HH:mm:ss", culture)}'",
            TimeSpan ts => $"'{ts.ToString("c", culture)}'",
            Guid g => $"'{g}'",
            byte[] bytes => $"X'{Convert.ToHexString(bytes)}'",
            // Round-trip formats, so a default written here reads back as the value it was given.
            float f => f.ToString("R", culture),
            double dbl => dbl.ToString("R", culture),
