...
public sealed class WitSqlEngineStatement : IDisposable
{
    ...
    private WitSqlResult ExecuteInternal(ContextExecution context, CancellationToken cancellationToken)
    {
        ...
        // The engine's own execute path publishes these after running; this one did not, so the
        // key an INSERT had just generated was unreachable through a prepared statement and every
        // caller that asked for it got zero.
    ...
    /// <summary>
    /// Gets the row id generated by the most recent INSERT executed through this statement.
    /// </summary>
    public long LastInsertRowId { get; private set; }

    /// <summary>
    /// Gets the number of rows changed by the most recent execution of this statement.
    /// </summary>
