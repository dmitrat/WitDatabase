...
>
> **55 `[Ignore(…)]` attributes plus 14 `[TestCase(… Ignore =)]` — 69 suppressed entries**, and **2**
> `[Explicit]`, counted 2026-07-30 across the second half's seven PRs (three row-lock markers, the MVCC
> deadlock detector, the deleted page-latch subsystem, the page-cache corruption window, the two LSM
> read-path markers that turned out to be a data-loss defect, and three parallel-access markers whose
> reason string PR #56 had already made false; it was 66 + 14 = 80 at the close of the first half).
> **The counting method was wrong three times
